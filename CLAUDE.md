# kotlin-ai Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-03-26

## Active Technologies
- Kotlin 1.9.25 / JVM 21 + Spring Boot 3.5.11, Spring Data JPA, Spring Web (RestClient), springdoc-openapi 2.8.6, `jackson-dataformat-xml` (NEW) (003-job-ingestion-pipeline)
- H2 (dev), `ddl-auto: create-drop` (003-job-ingestion-pipeline)
- Kotlin 1.9.25 / JVM 21 + Spring Boot 3.5.11, Spring Data JPA, Spring Web (RestClient), springdoc-openapi 2.8.6, jackson-dataformat-xml, PostgreSQL pgvector extension, pg_trgm extension (004-job-rag-search)
- PostgreSQL (prod/local via AWS RDS), H2 (dev/test — limited: no pgvector support) (004-job-rag-search)
- Kotlin 1.9.25 / JVM 21 + Spring Boot 3.5.11, Spring Data JPA, springdoc-openapi 2.8.6 (005-user-search-history)
- PostgreSQL (prod/local), H2 (dev/test) (005-user-search-history)

- Kotlin 1.9.25 / JVM 21 (existing) + Spring Boot 3.x, Spring Data JPA, springdoc-openapi (existing) (001-job-search-api)

## Project Structure

```text
src/main/kotlin/org/example/kotlinai/
├── controller/     ← @RestController (HTTP only, no business logic)
├── service/        ← @Service + toResponse() extension fns
├── repository/     ← Spring Data JPA interfaces
├── entity/         ← @Entity classes
├── dto/request/    ← inbound data classes
├── dto/response/   ← outbound data classes
└── global/exception/ ← GlobalExceptionHandler, ErrorResponse
src/test/kotlin/    ← mirrors main package structure
backend/
frontend/
tests/
```

## Commands

```bash
./gradlew bootRun --args='--spring.profiles.active=local'   # run locally with API key
./gradlew test                                               # run tests
./gradlew build                                              # build JAR
```

## Code Style

- 4-layer architecture: Controller → Service → Repository → Entity (enforced by constitution)
- `require()` for validation, `orElseThrow {}` for Optional unwrapping
- `@Transactional(readOnly = true)` at class level, override `@Transactional` on write methods
- All exceptions mapped in `GlobalExceptionHandler` (no inline try/catch in services/controllers)
- Expression bodies for single-expression functions

## Recent Changes
- 005-user-search-history: Added Kotlin 1.9.25 / JVM 21 + Spring Boot 3.5.11, Spring Data JPA, springdoc-openapi 2.8.6
- 005-user-search-history: Added Kotlin 1.9.25 / JVM 21 + Spring Boot 3.5.11, Spring Data JPA, springdoc-openapi 2.8.6
- 004-job-rag-search: Added Kotlin 1.9.25 / JVM 21 + Spring Boot 3.5.11, Spring Data JPA, Spring Web (RestClient), springdoc-openapi 2.8.6, jackson-dataformat-xml, PostgreSQL pgvector extension, pg_trgm extension


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
